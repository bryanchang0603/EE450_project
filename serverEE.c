 ...
 */
struct ee_course_node
{
    char course_code[50]; // need to specify the length of this
    char credit[50];
    char professor[100];   // need to specify the length
    char lecture_day[50];  // need to specify the length
    char course_name[100]; // need to specify the length
    struct ee_course_node *next;
};

struct ee_course_node *head;
struct ee_course_node *current;
...
/**
 * @brief append the new ee course to the front of the linked list
 *
 ...
 */
void EE_append_front(char course_code_in[50], char credit_in[10], char professor_in[100],
                     char lecture_day_in[50], char course_name_in[100])
{
    struct ee_course_node *temp_ee_node = (struct ee_course_node *)malloc(sizeof(struct ee_course_node));
    strcpy(temp_ee_node->course_code, course_code_in);
    strcpy(temp_ee_node->credit, credit_in);
    strcpy(temp_ee_node->professor, professor_in);
    strcpy(temp_ee_node->lecture_day, lecture_day_in);
    strcpy(temp_ee_node->course_name, course_name_in);
    temp_ee_node->next = head;
    head = temp_ee_node;
};
...
void print_all()
{
    struct ee_course_node *temp = head;
    while (temp != NULL)
    {
        current = temp;
        temp = temp->next;
...
char *multi_course_query(char *course_code_in)
{
    ...
    char *EE_query_output = (char *)malloc(sizeof(char) * 0);
    EE_query_output[0] = '\0';
    int output_length = 0;
    ...
    while (single_course_code != NULL)
    {
        ...
        while (temp != NULL)
        {
            ...
            if (strcmp(current->course_code, single_course_code) == 0)
            {
                ...
                EE_query_output = realloc(EE_query_output, output_length * (sizeof(char *)));
                strcat(EE_query_output, single_course_code);
                strcat(EE_query_output, ":");
                ...
                EE_query_output = realloc(EE_query_output, output_length * (sizeof(char *)));
                strcat(EE_query_output, course_info_result);
                strcat(EE_query_output, ",");
...
int main(int argc, char *argv[])
{
    ...
    {
        // process the line
        code_buffer = strtok(ee_course_buffer, ",");
        ...
        EE_append_front(code_buffer, credit_buffer, professor_buffer,
                        lecture_day_buffer, course_buffer);
    };
    fclose(ee_file);
    ...
    //     {
    //         print_all();
    //         test_result = find_course_info("EE450", "Credit");
    //         printf("%s\n", test_result);
    //         test_result = find_course_info("EE451", "Credit");
    //         printf("%s\n", test_result);
    //         test_result = find_course_info("EE450", "credit");
    //         printf("%s\n", test_result);
    //         test_result = find_course_info("EE450", "Professor");
    //         printf("%s\n", test_result);
    //         test_result = find_course_info("EE520", "Days");
    //         printf("%s\n", test_result);
    //         test_result = find_course_info("EE608", "CourseName");
    //         printf("%s\n", test_result);
    ...
    if ((rv = getaddrinfo("localhost", UDP_port_EE, &hints, &serverinfo)) != 0)
    {
